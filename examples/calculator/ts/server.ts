...
function topla(args: any): unknown[] {
  const nums = args?.sayilar;
  if (!Array.isArray(nums) || !nums.every((n) => typeof n === "number"))
    return [{ type: "text", text: "hata: sayilar dizi olmali: [1, 2, 3]" }];
  return [{ type: "text", text: `toplam: ${nums.reduce((a, b) => a + b, 0)}` }];
}

function carp(args: any): unknown[] {
  const nums = args?.sayilar;
  if (!Array.isArray(nums) || !nums.every((n) => typeof n === "number"))
    return [{ type: "text", text: "hata: sayilar dizi olmali: [1, 2, 3]" }];
  return [{ type: "text", text: `carpim: ${nums.reduce((a, b) => a * b, 1)}` }];
}
...
const TOOLS: Record<string, ToolSpec> = {
  topla: {
    ...
    inputSchema: { type: "object", properties: { sayilar: { type: "array", items: { type: "number" } } }, required: ["sayilar"] },
    handler: topla,
  },
  carp: {
